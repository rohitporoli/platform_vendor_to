...
PRODUCT_VERSION_MILESTONE = M6-FINAL
...
ifeq ($(TO_BUILDTYPE), RELEASE)
    ifndef TARGET_VENDOR_RELEASE_BUILD_ID
        ifeq ($(PRODUCT_VERSION_MAINTENANCE),0)
            TO_VERSION := OCT-L-$(PRODUCT_VERSION_MILESTONE)-$(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR)-$(shell date -u +%Y%m%d)$(PRODUCT_VERSION_DEVICE_SPECIFIC)
        else
            TO_VERSION := OCT-L-$(PRODUCT_VERSION_MILESTONE)-$(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR).$(PRODUCT_VERSION_MAINTENANCE)-$(shell date -u +%Y%m%d)$(PRODUCT_VERSION_DEVICE_SPECIFIC)
        endif
    else
        ifeq ($(TARGET_BUILD_VARIANT),user)
            TO_VERSION := OCT-L-v$(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR)-$(TARGET_VENDOR_RELEASE_BUILD_ID)
        else
            TO_VERSION := OCT-L-v$(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR).$(PRODUCT_VERSION_MAINTENANCE)$(PRODUCT_VERSION_DEVICE_SPECIFIC)
        endif
    endif
else
